...
window.DocumentosAEB.gerarFichaCadastro = function(dados) {
  ...
  // 1. LÓGICA DA IDENTIFICAÇÃO (CIN vs CPF/RG)
  let linhaIdentificacao;
  let docAssinatura;
  
  if (dados.tipoDoc === 'cin') {
      linhaIdentificacao = [ { text: 'CIN nº: ', bold: true }, dados.cpf ];
      docAssinatura = dados.cpf;
  } else {
      linhaIdentificacao = [ { text: 'CPF nº: ', bold: true }, dados.cpf, { text: '    RG nº: ', bold: true }, dados.rg ];
      docAssinatura = dados.rg;
  }

  // Se for menor de idade, quem assina é o responsável
  if (!dados.isMaior) {
      docAssinatura = dados.rgResp;
  }

  // 2. LÓGICA DA FOTO 3x4 (Embutindo a imagem direto no PDF)
  let blocoFoto = (dados.isNovo && dados.fotoBase64) 
      ? { image: dados.fotoBase64, width: 75, height: 100, alignment: 'right' } 
      : { width: 75, text: '' }; // Espaço vazio se não houver foto

  return {
    // Margens elegantes padrão A4
    pageMargins: [50, 40, 50, 40],
    
    content: [
      // CABEÇALHO COM FOTO AO LADO
      {
        columns: [
          { text: 'FICHA CADASTRAL AEB\n2º SEMESTRE DE 2026', style: 'header', width: '*', margin: [0, 20, 0, 0] },
          blocoFoto
        ],
        margin: [0, 0, 0, 20]
      },
    ...
    // DEFINIÇÃO GLOBAL DE ESTILOS (Deixa o documento bonito)
    defaultStyle: { 
        fontSize: 11, 
        lineHeight: 1.2 
    },
    styles: {
        ...
        justifiedText: { alignment: 'justify', fontSize: 10 }
    }
  };
};
